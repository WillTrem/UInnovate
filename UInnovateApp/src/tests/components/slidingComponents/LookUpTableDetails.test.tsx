...
describe('LookUpTableDetails component', () => {
  ...
  it('renders the component with data including "table1"', async () => {
    // Mock necessary functions for useEffect
    globalThis.fetch = async () => ({
      json: async () => [{ /* Mock row data */ }],
    } as Response);
    ...
    await waitFor(() => {
      // Check for the presence of "Missing ConnectionID"
      const missingConnectionIDElement = screen.getByText(/Missing ConnectionID/i);
      expect(missingConnectionIDElement).toBeInTheDocument();

      // Assert that your component renders the expected content
      const table1Element = screen.getByText((content) => {
        // Use a custom text matcher function to handle dynamic content
        const normalizedContent = content.trim().toLowerCase();
        return normalizedContent.includes('table1');
      });
