...
export async function fetchProcedureSource(schema: string, functionName: string): Promise<{source_code: string, arg_count: number}> {
  const func = await vmd.getFunctionAccessor('meta' || '', 'get_function_source_code_and_arg_count');
  const input = new Row();
...
export async function callProcedure(params: ProcedureSchedulingParams): Promise<void> {
  if(params.schema === undefined) {return;}
  const func = await vmd.getFunctionAccessor(params.schema || '', params.functionName);
  const input = new Row();
